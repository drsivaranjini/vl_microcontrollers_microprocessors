...
class Microcontroller:
    def __init__(self):
        ...
        # virtual-lab patch: optional callback fired on every external-memory
        # (MOVX) write, used to stream DAC writes out to the host page. See
        # public/emu/8051/PATCHES.md.
        self.xmem_write_hook = None
    ...
    def _exec_144(self, high_order_byte, low_order_byte):
        # virtual-lab patch: this took a single combined `immed` before, but next_cycle() always
        # calls _exec_N(*op.args) with the two raw trailing bytes un-combined (see _exec_2/_exec_18
        # for the established convention) — MOV DPTR,#data16 crashed on every real ROM step. Bug in
        # upstream i8051emu, unrelated to the MOVX hook patch; see PATCHES.md.
    ...
    def _exec_240(self):
        self._xmem_write(self.mem.dptr, self.mem.a)
    ...
    def _exec_242(self):
        self._xmem_write(int(self.mem.p2.bits + self.mem.r0.bits, 2), self.mem.a)

    def _exec_243(self):
        self._xmem_write(int(self.mem.p2.bits + self.mem.r1.bits, 2), self.mem.a)
